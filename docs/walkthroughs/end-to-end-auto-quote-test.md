...
- **CAD file:** upload **a simple known-good `.step` / `.stp` shape**. **The pipeline is STEP-only** — the OCCT-backed STEP path shipped in `PROD_v2.27.8` (S292 / PR-273, 2026-06-09) and `.stl` was retired at the storefront afterwards: an STL upload never reaches the extractor, the form rejects it with a per-file chip reading "STL is no longer accepted — please upload STEP (.step/.stp)". This also catches an STL merely renamed `.step` (content sniff), so re-naming is not a workaround — re-export. Assemblies (multi-solid STEP) are explicitly rejected: ensure your STEP is a single-part solid; the row will surface "STEP file contains an assembly with N solids" if not. A 30×30×30 mm cube is the safest first try. For v1 success, also avoid shapes with thin walls, 5-axis features, or internal pockets — the OCCT extractor is stubbed for many feature classes. **STEP unit-of-measure normalisation** (PROD_v2.27.9 / S297 F2) silently converts `LENGTH_UNIT('METRE')` / `'CENTI METRE'` / `'INCH'` files to mm at import — but if your CAD tool emits the geometry coords AND the LENGTH_UNIT inconsistently (a Fusion bug class), inspect the bbox in the Pricing-job row's detail pane before clicking DEAL. **Other formats** (`.iges`, `.dxf`, `.sldprt`, `.obj`, …) are rejected at upload as of S204 — the accepted set is `.step` / `.stp` only, and everything else comes back as a per-file chip reading "… is not accepted. We currently accept STEP only (.step / .stp). Please export or convert your CAD to STEP and re-upload." instead of dead-ending at the extractor with a `Permanent` badge. A non-STEP file merely renamed `.step` is caught by the same content sniff. Backlog D-18 (internal OCCT normalize) is what re-broadens the set to the OCCT-readable solid formats (IGES/BREP); mesh (`.stl`/`.3mf`/`.obj`) stays out permanently.
...
This email is sent by the **PR-07 fire-and-forget path** in `/api/quote`, rewired in PR-11 onto the **storefront email outbox queue** (ADR-0009). The handler returns 200 immediately and the enqueue runs via `setImmediate`; the customer never waits on the disk write. ABERP's poller picks the queue entry up on its next 60s cycle and SMTP-relays the message itself. **If it doesn't arrive, see Troubleshooting §"No email arrives" — but the customer still got their reference id on the confirmation page, so the rest of the walkthrough is unblocked.**
...
**[Email inbox]** Within ~60s after Step 3's `Posted` state (one ABERP poll cycle), a second email should arrive. Subject: `Ajánlat <UUID-short> — készen áll / Your quote is ready`.
...
**[Email inbox]** Within ~60s (one ABERP poll cycle), a third email arrives. Subject: HU+EN, "Thank you for accepting your quote."

This is the third queue-path validation (the first was Step 2's submission-received email, the second was Step 5's quote-ready). Three storefront-outbox entries consumed cleanly by ABERP's poller is the green-light signal for ADR-0009.
...
- **Fix (STEP file — assembly):** "STEP file contains an assembly with N solids; only single-part STEP is supported in v1" — the OCCT extractor (PROD_v2.27.8 / S292) rejects multi-solid STEP. Re-export from your CAD tool as a single-part STEP.
- **Fix (STEP file — no solid body):** "STEP file contains no solid body" — the file likely contains only surfaces / sheets / wireframes. Re-export with solid bodies as a single-part STEP.
- **Fix (STEP file — could not be parsed):** "STEP file could not be parsed (OCCT ReadFile status=N)" — file is malformed. Re-export a fresh STEP from a known-good tool (AP203/AP214/AP242 all parse).
- **Fix (unsupported extension):** "Unsupported file extension '.iges'…" — only reachable for rows submitted **before S204**. The storefront now accepts `.step` / `.stp` only, so a non-STEP file is rejected at upload with a per-file chip and never reaches the extractor. The `Permanent` badge tells the operator Retry will never help; the customer must re-upload as STEP.
...
- **Fix (other):** the CAD file is unparseable. Re-submit with a simpler shape (a single-part 30 mm cube STEP). If a previously-working shape now fails, the OCCT extractor regressed — file under `[[aberp-python-auto-discovery]]` follow-up.
...
- ADR-0007 — [Storefront email relay via ABERP](../adr/0007-storefront-email-relay-via-aberp.md) (relay endpoint contract, supersedes ADR-0006; superseded for the email leg by ADR-0009).
- ADR-0008 — [Storefront ↔ ABERP network topology](../adr/0008-aberp-storefront-network-topology.md) (Option B / Cloudflare Tunnel picked for the push legs; superseded for the email leg by ADR-0009).
- ADR-0009 — [Storefront-as-queue for outbound email](../adr/0009-storefront-as-queue-no-tunnel.md) (pull-based email outbox; supersedes ADR-0007's push shape).
