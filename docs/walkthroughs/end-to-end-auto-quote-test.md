...
- **Fix (unsupported extension):** "Unsupported file extension '.iges'…" — the storefront accepts 13 extensions at upload but the extractor only handles `.step` / `.stp`. The `Permanent` badge tells the operator Retry will never help; the customer must re-upload in a supported format.
