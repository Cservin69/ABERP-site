...
## Addendum — S356: quote state dir

The **customer-quote store** — the queue this ADR is fundamentally about — has the same canonical-state-dir requirement, and it is the most consequential of the three because, unlike the catalogue, it fails _silently_:

- The quote store lives at `/home/aberp/data/quotes/<id>/` (`metadata.json`, `priced.pdf`, `files/`), overridable via `ABERP_SITE_QUOTE_DIR` (**absolute paths only**). `resolveQuoteDir()` in `quote-store.ts` throws on a relative override; `F-QUOTE` in `boot-checks.ts` refuses startup (503) if the dir is missing, relative, or unwritable — mirroring `F15` (outbox) and `F-CAT` (catalogue).
- Before S356, `quote-store.ts` defaulted to the process-CWD-relative `./data/quotes`, which `pathResolve` anchored inside the immutable release dir. Unlike the catalogue (which 500s loudly on the `EROFS` write), the quote dir was _created fine_ inside the release dir and then **silently discarded on the next deploy**, taking every `metadata.json` with it. The downstream symptom was a `GET /api/quotes/{id}/priced` `404` that CloudFront's `404 → /index.html` rule masked as a `200 text/html` SPA shell — which ABERP's S347 writeback classifier then mislabelled as `RoutingMisconfigured` (a CDN routing fault), sending the operator to the wrong panel. S356 sets the absolute default `/home/aberp/data/quotes`, rejects relative overrides, and adds `F-QUOTE`. Operator deploy steps: `docs/walkthroughs/deploy-storefront.md`. CDN-side hardening (flip the masking `404 → 200` CustomErrorResponse back to `404`): `docs/walkthroughs/cloudfront-error-page-fix.md`. This is the quote-store analogue of the S311 (outbox) and S343 (catalogue) path-resolution fixes.
