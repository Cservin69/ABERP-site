...
| `ABERP_SITE_PUBLIC_URL`  | Public-facing base URL of this deployment (no trailing slash). Single source of truth for transactional emails, dynamic sitemap.xml + robots.txt, canonical / og:url meta, and the Origin allowlist in `src/lib/server/origin-check.ts`. Defaults to `https://abenerp.com`. **PR-Q** consolidated the legacy `ABERP_SITE_PUBLIC_URL` + `ABERP_SITE_PUBLIC_BASE_URL` split into this single var; only this name is honoured. |
