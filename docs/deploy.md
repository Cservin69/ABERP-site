...
`build/` must contain at least: `index.html`, `privacy.html`, `imprint.html`, `og-image.png`, `og-image.svg`, `favicon.svg`, `favicon-16x16.png`, `favicon-32x32.png`, `apple-touch-icon.png`, and the hashed `_app/` directory of compiled assets. `sitemap.xml` and `robots.txt` are now SvelteKit prerendered endpoints (PR-Q) — adapter-node writes them under `build/prerendered/`, served by the Node runtime.
...
ENV PORT=3000
ENV ABERP_SITE_QUOTE_DIR=/home/aberp/data/quotes
EXPOSE 3000
CMD ["node", "build"]
```
...
| `NODE_ENV`               | `production`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `ABERP_SITE_QUOTE_DIR`   | Absolute path where quote submissions are staged. Default `/home/aberp/data/quotes` (S356 — the canonical state dir whitelisted by the systemd unit's `ReadWritePaths=`). **Must be absolute** — `resolveQuoteDir()` throws on a relative override, because a CWD-relative path lands inside the immutable release dir and is wiped on every redeploy (the [[s356]] root cause). Boot-checked by `F-QUOTE` (refuse-to-start if unwritable). See `docs/walkthroughs/deploy-storefront.md`.                                                                                                                                                                                                                                                            |
...
| `ABERP_SITE_PUBLIC_URL`  | **REQUIRED in production.** Public-facing base URL of this deployment (no trailing slash). Single source of truth for transactional emails, dynamic sitemap.xml + robots.txt, canonical / og:url meta, and the Origin allowlist in `src/lib/server/origin-check.ts`. **PR-R** makes `publicSiteUrl()` throw on first call in production when this is unset/blank — a misconfigured staging box now refuses state-changing POSTs instead of fail-opening with the hardcoded prod URL. Dev + `npm run build` prerender still fall back to `https://abenerp.com` so CI works without injecting the var. **PR-Q** consolidated the legacy `ABERP_SITE_PUBLIC_URL` + `ABERP_SITE_PUBLIC_BASE_URL` split into this single var; only this name is honoured. |
| `ORIGIN`                 | SvelteKit's standard origin pin. Set to the same value as `ABERP_SITE_PUBLIC_URL` so the built-in `csrf.checkOrigin` matches the browser's `Origin` header behind CloudFront. See [[quote-csrf-origin]] — a divergence here was the root cause of the live "form POSTs to /" 403 fixed in PR-N.                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
