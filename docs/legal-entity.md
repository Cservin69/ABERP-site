# Legal entity — fields required for imprint

The imprint (`/imprint`) page is required under EU/Hungarian law for any commercial website. Below is the field list; items marked **[PLACEHOLDER]** must be filled by Ervin before public launch.
...
- VAT identifier (if different from Tax ID for cross-border invoicing).
- Telephone number — only if Ervin wants to publish one. Not required by Hungarian imprint law if email contact is provided.
- Supervisory authority — only required for regulated professions; manufacturing brokerage is not currently regulated in this sense.
