...
>
	...
	{#if showMonogram}
		<!--
		  Áben mark — the approved traced Á, identical artwork to
		  static/brand-mark.svg (verbatim facets + source-sampled gradients).
		  Built X-like: the left and lower-right arms (armL/armR) form the
		  letterform; the upper-right arm escaped and became the acute accent.
		  Embedded as a nested <svg> whose viewBox tightly frames the inked mark
		  ("160 352 616 436" of the source's 768×1152 space) so it drops centred
		  above the ABENERP wordmark with no transform math. Renders only on the
		  dark hero here; the mark is approved on black and white both.
		-->
		<svg
			class="mark"
			x="332"
			...
			width="136"
			height="96"
			viewBox="160 352 616 436"
			role="img"
			aria-label="Áben mark"
		>
			<defs>
				<linearGradient
					id="gArmL"
